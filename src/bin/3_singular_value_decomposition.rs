...
use learning_lm::{jacobi_svd, norm, sub, svd_lstsq, Mat};
use rand::prelude::*;
...
fn main() {
    ...
    let recon_err = u.matmul(&Mat::diag(&sigma)).matmul(&v.transpose()).sub(&a).frobenius_norm();
    ...
    let res = sub(&b, &a_def.matvec(&x_mn));
...
mod tests {
    ...
    fn test_svd_properties() {
        ...
        for (n, m) in [(3, 3), (5, 3), (10, 4), (20, 7)] {
            ...
            // A = U Σ Vᵀ
            let recon_err =
                u.matmul(&Mat::diag(&sigma)).matmul(&v.transpose()).sub(&a).frobenius_norm();
    ...
    fn test_min_norm_solution() {
        ...
        // 残差はゼロ (b は列空間内)
        assert!(norm(&sub(&b, &a.matvec(&x))) < 1e-10);
